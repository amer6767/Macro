--[[
    ╔═══════════════════════════════════════════════════════════════════════════╗
    ║                    SCRIPT EXPLORER v10.1 CLEAN EDITION                    ║
    ║                        Modular • Optimized • Clean                        ║
    ╠═══════════════════════════════════════════════════════════════════════════╣
    ║  • Proper modular architecture (UI, State, Scanner, Decompiler)           ║
    ║  • No code duplication - DRY principles                                   ║
    ║  • Consistent executor support checks                                     ║
    ║  • Optimized loops and memory usage                                       ║
...
local Config = {
    ...
    Tree = { PanelWidth = 0.6, ItemHeight = 34, IndentSize = 18, MaxNodes = 5000, MaxDepth = 40 },
    ...
    Icons = {
        ...
        Expand = "▼", Collapse = "▶", Copy = "📋", Refresh = "🔄", Star = "⭐",
    },
    
    Services = {"Workspace", "ReplicatedStorage", "ReplicatedFirst", "Players", 
                "StarterGui", "StarterPack", "StarterPlayer", "Lighting", "Chat"},
}
...
function Executor.has(funcName)
    return type(getfenv()[funcName]) == "function"
end

function Executor.call(funcName, ...)
    if not Executor.has(funcName) then return nil end
    local success, result = pcall(getfenv()[funcName], ...)
    return success and result or nil
end

function Executor.clipboard(text)
    local fn = Executor.has("setclipboard") and setclipboard 
            or Executor.has("toclipboard") and toclipboard
...
local State = {
    ...
    billboard = nil,
    favorites = {},
    filters = { scripts = true, modules = true, locals = true, all = true },
}
...
function State.addFavorite(instance)
    local path = instance:GetFullName()
    for _, f in ipairs(State.favorites) do
        if f.path == path then return false end
    end
    table.insert(State.favorites, { name = instance.Name, path = path, class = instance.ClassName })
    return true
end
...
function UI.padding(parent, size)
    ...
    p.PaddingTop, p.PaddingBottom = UDim.new(0, size), UDim.new(0, size)
    p.PaddingLeft, p.PaddingRight = UDim.new(0, size), UDim.new(0, size)
    p.Parent = parent
    return p
end

function UI.list(parent, padding, direction)
...
function UI.tween(instance, props, duration)
    local ti = TweenInfo.new(duration or 0.12, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
...
function UI.frame(props)
    ...
    if props.Stroke then UI.stroke(f, props.Stroke.Size, props.Stroke.Color, props.Stroke.Trans) end
...
function UI.label(props)
    ...
    if props.Wrap then l.TextWrapped = true end
...
function UI.button(props)
    ...
    local hoverStroke = UI.stroke(b, 2, Config.Colors.Accent, 1)
    b.MouseEnter:Connect(function() UI.tween(hoverStroke, {Transparency = 0.3}, 0.1) end)
    b.MouseLeave:Connect(function() UI.tween(hoverStroke, {Transparency = 1}, 0.1) end)
...
function Toast.init(parent)
    Toast.container = UI.frame({ Size = UDim2.new(0, 320, 1, 0), Position = UDim2.new(1, -330, 0, 0), Transparency = 1, Parent = parent })
    UI.list(Toast.container, 8)
    UI.padding(Toast.container, 15)
end

function Toast.show(message, duration, color)
    if not Toast.container then return end
    
    local toast = UI.frame({ Size = UDim2.new(1, 0, 0, 0), Color = color or Config.Colors.Panel, Corner = 10, Parent = Toast.container })
    toast.AutomaticSize = Enum.AutomaticSize.Y
    UI.stroke(toast, 1, Config.Colors.Accent, 0.5)
    
    local text = UI.label({ Text = message, Size = UDim2.new(1, -16, 0, 0), Position = UDim2.new(0, 8, 0, 8), 
                           Wrap = true, Parent = toast, Bold = true, TextSize = 13 })
    text.AutomaticSize = Enum.AutomaticSize.Y
    
    local pad = UI.frame({ Size = UDim2.new(1, 0, 0, 8), Transparency = 1, Parent = text })
    
    task.delay(duration or 3, function()
        if toast.Parent then
            UI.tween(toast, {BackgroundTransparency = 1}, 0.3)
            UI.tween(text, {TextTransparency = 1}, 0.3)
            task.wait(0.3)
...
local function serializeTable(tbl, depth)
    ...
    for k, v in pairs(tbl) do
        local key = type(k) == "string" and (k:match("^[%a_][%w_]*$") and k or '["'..k..'"]') or "["..tostring(k).."]"
        local val
        
        if type(v) == "string" then
            val = '"' .. v:sub(1, 80):gsub('"', '\\"'):gsub("\n", "\\n") .. (v:len() > 80 and "..." or "") .. '"'
        ...
        if #parts > 25 then table.insert(parts, indent .. "-- ... more entries") break end
    end
...
function Decompiler.decompileModule(moduleScript)
    local ok, module = pcall(require, moduleScript)
    if not ok or module == nil then return nil end
    
    local result = "-- Decompiled ModuleScript (speed's decompiler)\n"
    result = result .. "-- Path: " .. moduleScript:GetFullName() .. "\n\n"
    
    if type(module) == "table" then
        result = result .. "return " .. serializeTable(module, 1)
    elseif type(module) == "function" then
        result = result .. "return function() end -- compiled"
    else
...
function Decompiler.getSource(script)
    local methods = {
        ...
        function() 
            local closure = Executor.call("getscriptclosure", script)
            return closure and Executor.call("decompile", closure)
        end,
        function()
        ...
        function()
            if script:IsA("ModuleScript") then return Decompiler.decompileModule(script) end
        end,
    }
...
function Highlighter.clear()
    ...
    if State.billboard then State.billboard:Destroy() State.billboard = nil end
end

function Highlighter.show(instance)
    Highlighter.clear()
    
    local target = instance:IsA("BasePart") and instance 
    ...
    State.billboard.Size = UDim2.new(0, 300, 0, 60)
    State.billboard.StudsOffset = Vector3.new(0, 5, 0)
    State.billboard.Adornee = target
    State.billboard.AlwaysOnTop = true
    State.billboard.Parent = CoreGui
    
    local bg = UI.frame({ Color = Color3.new(0,0,0), Corner = 8, Parent = State.billboard })
    bg.BackgroundTransparency = 0.2
    UI.stroke(bg, 2, Color3.fromRGB(255, 255, 0))
    
    UI.label({ Text = "📍 " .. instance.Name .. " [" .. instance.ClassName .. "]", 
              Size = UDim2.new(1, -12, 0, 25), Position = UDim2.new(0, 6, 0, 4),
              Color = Color3.fromRGB(255, 255, 100), Bold = true, TextSize = 16, Parent = bg })
    
    UI.label({ Text = instance:GetFullName(), Size = UDim2.new(1, -12, 0, 20), Position = UDim2.new(0, 6, 0, 32),
              Color = Color3.fromRGB(200, 200, 200), TextSize = 11, Truncate = Enum.TextTruncate.AtEnd, Parent = bg })
...
function Scanner.getIcon(instance)
    return Config.Icons[instance.ClassName] or 
           (instance:IsA("BaseScript") and Config.Icons.Script) or
           (instance:IsA("BasePart") and Config.Icons.Part) or
           (instance:IsA("Folder") and Config.Icons.Folder) or
           (instance:IsA("Model") and Config.Icons.Model) or
           Config.Icons.Default
end

function Scanner.getColor(instance)
...
function Scanner.generateMap()
    local maps, totalObjs, totalScripts = {}, 0, 0
    ...
    for _, serviceName in ipairs(Config.Services) do
        ...
        local function scan(inst, depth, path)
            ...
            table.insert(lines, string.rep("  ", depth) .. icon .. " " .. inst.Name .. " [" .. inst.ClassName .. "] -- " .. path)
            
            for _, child in ipairs(inst:GetChildren()) do
                pcall(scan, child, depth + 1, path .. "." .. child.Name)
            end
        end
        
        for _, child in ipairs(service:GetChildren()) do
            pcall(scan, child, 0, serviceName .. "." .. child.Name)
        end
        
        totalObjs, totalScripts = totalObjs + objs, totalScripts + scripts
        maps[serviceName] = { lines = lines, objs = objs, scripts = scripts }
    end
    
    return maps, totalObjs, totalScripts
end

function Scanner.search(query, parent)
    local results = {}
    query = query:lower()
    ...
    for _, serviceName in ipairs(Config.Services) do
        ...
        local ok, service = pcall(game.GetService, game, serviceName)
        if ok and service then pcall(searchIn, service) end
    end
...
function Explorer.selectItem(instance)
    ...
    if State.pathLabel then State.pathLabel.Text = "📍 " .. instance:GetFullName() end
    
    local icon = Scanner.getIcon(instance)
    if State.titleLabel then State.titleLabel.Text = icon .. " " .. instance.Name end
    
    Highlighter.show(instance)
    Toast.show(icon .. " " .. instance.Name .. "\n[" .. instance.ClassName .. "]\n" .. instance:GetFullName(), 3)
...
function Explorer.createNode(instance, parent, indent, order)
    ...
    local container = UI.frame({ Size = UDim2.new(1, 0, 0, Config.Tree.ItemHeight), Transparency = 1, Parent = parent })
    ...
    local icon = Scanner.getIcon(instance)
    local expand = hasChildren and Config.Icons.Collapse or ""
    btn.Text = "  " .. expand .. " " .. icon .. " " .. instance.Name .. 
               " <font color=\"#666\">[" .. instance.ClassName .. "]</font>" ..
               (hasChildren and " <font color=\"#555\">(" .. childCount .. ")</font>" or "")
    
    local hoverStroke = UI.stroke(btn, 2, Config.Colors.Accent, 1)
    btn.MouseEnter:Connect(function() UI.tween(btn, {BackgroundTransparency = 0.7}, 0.1) UI.tween(hoverStroke, {Transparency = 0.3}, 0.1) end)
    btn.MouseLeave:Connect(function() UI.tween(btn, {BackgroundTransparency = 0.9}, 0.1) UI.tween(hoverStroke, {Transparency = 1}, 0.1) end)
    
    local childrenFrame = UI.frame({ Size = UDim2.new(1, 0, 0, 0), Position = UDim2.new(0, 0, 0, Config.Tree.ItemHeight), Transparency = 1, Parent = container })
    childrenFrame.Visible = false
    childrenFrame.AutomaticSize = Enum.AutomaticSize.Y
    UI.list(childrenFrame, 2)
    
    local isExpanded, childrenLoaded = false, false
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if hasChildren then
            ...
            expand = isExpanded and Config.Icons.Expand or Config.Icons.Collapse
            btn.Text = "  " .. expand .. " " .. icon .. " " .. instance.Name .. 
                       " <font color=\"#666\">[" .. instance.ClassName .. "]</font>" ..
            ...
            if isExpanded and not childrenLoaded then
                ...
                table.sort(children, function(a, b)
                    ...
                    return aScore == bScore and a.Name < b.Name or aScore < bScore
                end)
                
                for i, child in ipairs(children) do
                    pcall(Explorer.createNode, child, childrenFrame, indent + 1, i)
                end
            end
        end
    end)
...
function Explorer.buildTree(query)
    ...
    if query and #query >= Config.Search.MinChars then
        ...
        for i, instance in ipairs(results) do
            pcall(Explorer.createNode, instance, State.treeScroll, 0, i)
        end
    else
        local order = 0
        for _, serviceName in ipairs(Config.Services) do
            ...
            local serviceNode = UI.frame({ Size = UDim2.new(1, 0, 0, Config.Tree.ItemHeight), Transparency = 1, Parent = State.treeScroll })
            ...
            UI.corner(serviceBtn, 6)
            
            local serviceChildren = UI.frame({ Size = UDim2.new(1, 0, 0, 0), Position = UDim2.new(0, 0, 0, Config.Tree.ItemHeight), Transparency = 1, Parent = serviceNode })
            serviceChildren.Visible = false
            serviceChildren.AutomaticSize = Enum.AutomaticSize.Y
            UI.list(serviceChildren, 2)
            
            local serviceExpanded, serviceLoaded = false, false
            ...
            serviceBtn.MouseButton1Click:Connect(function()
                ...
                if serviceExpanded and not serviceLoaded then
                    ...
                    for i, child in ipairs(children) do
                        pcall(Explorer.createNode, child, serviceChildren, 1, i)
                    end
                end
            end)
        end
...
function GameMap.show()
    ...
    if not mapGui.Parent then mapGui.Parent = Players.LocalPlayer:WaitForChild("PlayerGui") end
    
    local mapFrame = UI.frame({ Size = UDim2.new(0.85, 0, 0.9, 0), Position = UDim2.new(0.075, 0, 0.05, 0), 
                                Color = Config.Colors.Background, Corner = 16, Parent = mapGui })
    UI.stroke(mapFrame, 3, Config.Colors.Accent, 0.2)
    
    local header = UI.frame({ Size = UDim2.new(1, 0, 0, 55), Color = Config.Colors.Panel, Corner = 16, Parent = mapFrame })
    UI.label({ Text = "🗺️ GAME MAP - Full Object List", Size = UDim2.new(1, -250, 1, 0), Position = UDim2.new(0, 20, 0, 0), Bold = true, TextSize = 20, Parent = header })
    
    UI.button({ Text = "✕", Size = UDim2.new(0, 45, 0, 45), Position = UDim2.new(1, -50, 0, 5), Color = Config.Colors.Error, 
               OnClick = function() mapGui:Destroy() end, Parent = header })
    
    UI.button({ Text = "✅ Open Explorer", Size = UDim2.new(0, 150, 0, 38), Position = UDim2.new(1, -210, 0, 8), Color = Config.Colors.Success, TextSize = 14,
               OnClick = function() mapGui:Destroy() end, Parent = header })
    
    local loading = UI.label({ Text = "🔄 Scanning game...", Size = UDim2.new(1, 0, 1, -55), Position = UDim2.new(0, 0, 0, 55), 
                               AlignX = Enum.TextXAlignment.Center, TextSize = 22, Color = Config.Colors.Muted, Parent = mapFrame })
    ...
    task.spawn(function()
        ...
        local stats = UI.frame({ Size = UDim2.new(1, -20, 0, 32), Position = UDim2.new(0, 10, 0, 62), Color = Config.Colors.Item, Corner = 8, Parent = mapFrame })
        UI.label({ Text = "📊 Total: " .. totalObjs .. " objects | " .. totalScripts .. " scripts", Size = UDim2.new(1, -20, 1, 0), Position = UDim2.new(0, 10, 0, 0), Color = Config.Colors.Accent, Bold = true, Parent = stats })
        
        local tabs = UI.scroll({ Size = UDim2.new(1, -20, 0, 42), Position = UDim2.new(0, 10, 0, 100), Parent = mapFrame })
        tabs.ScrollBarThickness = 0
        tabs.AutomaticCanvasSize = Enum.AutomaticSize.X
        UI.list(tabs, 5, Enum.FillDirection.Horizontal)
        
        local content = UI.scroll({ Size = UDim2.new(1, -20, 1, -155), Position = UDim2.new(0, 10, 0, 148), Parent = mapFrame })
        ...
        local function showService(serviceName)
            if currentTab then currentTab.BackgroundColor3 = Config.Colors.Item end
            ...
            if data then
                ...
                contentBox.Text = header .. table.concat(data.lines, "\n")
            end
        end
        ...
        local order = 0
        for _, serviceName in ipairs(Config.Services) do
            if maps[serviceName] and #maps[serviceName].lines > 0 then
                order = order + 1
                local tabBtn = UI.button({ Text = "📂 " .. serviceName:sub(1, 12), Size = UDim2.new(0, 120, 0, 34), Color = Config.Colors.Item, TextSize = 11, Corner = 6, Parent = tabs })
        ...
        -- Copy state for chunked copying
        local copyTabIndex = 0
        local copyAllIndex = 0
        local copyAllLines = {}
        local LINES_PER_COPY = 100
        
        -- Copy Current Tab Button (100 lines per click)
        local copyCurrent = UI.button({ Text = "📋 Copy 1-100", Size = UDim2.new(0, 100, 0, 34), Color = Config.Colors.Accent, TextSize = 10, Corner = 6, Parent = tabs })
        ...
        copyCurrent.MouseButton1Click:Connect(function()
            if currentServiceName and maps[currentServiceName] then
                ...
                local header = "-- " .. currentServiceName .. " [Lines " .. startLine .. "-" .. endLine .. " of " .. totalLines .. "]\n\n"
                Executor.clipboard(header .. table.concat(chunk, "\n"))
        ...
        -- Reset tab copy when switching tabs
        local origShowService = showService
        showService = function(serviceName)
            copyTabIndex = 0
            if maps[serviceName] then
                local totalLines = #maps[serviceName].lines
                local endLine = math.min(LINES_PER_COPY, totalLines)
                copyCurrent.Text = "📋 Copy 1-" .. endLine
            end
            origShowService(serviceName)
        end
        
        -- Copy All Button (100 lines per click across all services)
        local copyAll = UI.button({ Text = "📋 All 1-100", Size = UDim2.new(0, 100, 0, 34), Color = Config.Colors.Success, TextSize = 10, Corner = 6, Parent = tabs })
        copyAll.LayoutOrder = 999
        copyAll.MouseButton1Click:Connect(function()
            -- Build full lines array if empty or reset
            if #copyAllLines == 0 then
                copyAllLines = {"-- FULL GAME MAP", "-- Generated by Script Explorer v10.1", ""}
                for _, sn in ipairs(Config.Services) do
                    if maps[sn] and #maps[sn].lines > 0 then
                        table.insert(copyAllLines, "-- ═══════════════════════════════════════")
                        table.insert(copyAllLines, "-- " .. sn .. " (" .. maps[sn].objs .. " objects, " .. maps[sn].scripts .. " scripts)")
                        table.insert(copyAllLines, "-- ═══════════════════════════════════════")
                        table.insert(copyAllLines, "")
                        for _, line in ipairs(maps[sn].lines) do
            ...
            local header = "-- [Lines " .. startLine .. "-" .. endLine .. " of " .. totalLines .. "]\n\n"
            Executor.clipboard(header .. table.concat(chunk, "\n"))
...
local function init()
    ...
    if not State.gui.Parent then State.gui.Parent = LocalPlayer:WaitForChild("PlayerGui") end
    ...
    State.mainFrame = UI.frame({ 
        ...
        Color = Config.Colors.Background, Corner = 14, Clip = true, Parent = State.gui 
    })
    State.mainFrame.Active = true
    UI.stroke(State.mainFrame, 2, Config.Colors.Accent, 0.3)
    
    -- Header
    local header = UI.frame({ Size = UDim2.new(1, 0, 0, 52), Color = Config.Colors.Panel, Corner = 14, Parent = State.mainFrame })
    UI.label({ Text = "🚀 Script Explorer v" .. Config.Version, Size = UDim2.new(1, -180, 1, 0), Position = UDim2.new(0, 18, 0, 0), Bold = true, TextSize = 18, Parent = header })
    
    UI.button({ Text = "🗺️", Size = UDim2.new(0, 42, 0, 42), Position = UDim2.new(1, -175, 0, 5), OnClick = GameMap.show, Parent = header })
    
    local isMin = false
    local minBtn = UI.button({ Text = "─", Size = UDim2.new(0, 42, 0, 42), Position = UDim2.new(1, -130, 0, 5), Color = Config.Colors.Folder, Parent = header })
    minBtn.MouseButton1Click:Connect(function()
        isMin = not isMin
        UI.tween(State.mainFrame, {Size = isMin and UDim2.new(0, 350, 0, 52) or UDim2.fromScale(Config.Window.Width, Config.Window.Height)}, 0.2)
    end)
    
    UI.button({ Text = "✕", Size = UDim2.new(0, 42, 0, 42), Position = UDim2.new(1, -85, 0, 5), Color = Config.Colors.Error,
               OnClick = function() Highlighter.clear() State.gui:Destroy() end, Parent = header })
    
    -- Toolbar
    local toolbar = UI.frame({ Size = UDim2.new(1, -20, 0, 42), Position = UDim2.new(0, 10, 0, 58), Transparency = 1, Parent = State.mainFrame })
    
    local searchBox = UI.input({ Size = UDim2.new(1, -110, 1, 0), Placeholder = "🔍 Search...", Parent = toolbar })
    UI.button({ Text = "🔄", Size = UDim2.new(0, 42, 1, 0), Position = UDim2.new(1, -100, 0, 0), OnClick = function() Explorer.buildTree() end, Parent = toolbar })
    UI.button({ Text = "⭐", Size = UDim2.new(0, 42, 1, 0), Position = UDim2.new(1, -52, 0, 0), Color = Config.Colors.Folder, Parent = toolbar })
    
    local debounce = nil
    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        local q = searchBox.Text
        if debounce then task.cancel(debounce) end
        debounce = task.delay(Config.Search.Debounce, function() Explorer.buildTree(q) end)
    end)
    
    -- Path bar
    local pathBar = UI.frame({ Size = UDim2.new(1, -20, 0, 28), Position = UDim2.new(0, 10, 0, 106), Color = Config.Colors.Item, Corner = 8, Parent = State.mainFrame })
    State.pathLabel = UI.label({ Text = "📍 Select an item...", Size = UDim2.new(1, -90, 1, 0), Position = UDim2.new(0, 12, 0, 0), Color = Config.Colors.Muted, TextSize = 12, Truncate = Enum.TextTruncate.AtEnd, Parent = pathBar })
    
    UI.button({ Text = "📋", Size = UDim2.new(0, 70, 0, 22), Position = UDim2.new(1, -78, 0, 3), TextSize = 12, Corner = 6,
               OnClick = function() if State.selected then Executor.clipboard(State.selected:GetFullName()) Toast.show("📋 Path copied!", 2) end end, Parent = pathBar })
    
    -- Split container
    local split = UI.frame({ Size = UDim2.new(1, -20, 1, -145), Position = UDim2.new(0, 10, 0, 140), Transparency = 1, Clip = true, Parent = State.mainFrame })
    
    -- Tree panel
    local treePanel = UI.frame({ Size = UDim2.new(Config.Tree.PanelWidth, -5, 1, 0), Color = Config.Colors.Panel, Corner = 10, Clip = true, Parent = split })
    State.treeScroll = UI.scroll({ Parent = treePanel })
    UI.list(State.treeScroll, 2)
    UI.padding(State.treeScroll, 6)
    
    -- Code panel
    local codePanel = UI.frame({ Size = UDim2.new(1 - Config.Tree.PanelWidth, -5, 1, 0), Position = UDim2.new(Config.Tree.PanelWidth, 5, 0, 0), Color = Config.Colors.Panel, Corner = 10, Clip = true, Parent = split })
    
    local codeHeader = UI.frame({ Size = UDim2.new(1, 0, 0, 45), Color = Config.Colors.Item, Parent = codePanel })
    State.titleLabel = UI.label({ Text = "📜 Select a script", Size = UDim2.new(1, -140, 1, 0), Position = UDim2.new(0, 12, 0, 0), Bold = true, TextSize = 14, Truncate = Enum.TextTruncate.AtEnd, Parent = codeHeader })
    
    UI.button({ Text = "▶", Size = UDim2.new(0, 38, 0, 34), Position = UDim2.new(1, -130, 0, 5), Color = Config.Colors.Success, TextSize = 14,
               OnClick = function()
                   if State.source then
                       local fn, err = loadstring(State.source)
                       if fn then pcall(fn) Toast.show("✅ Executed!", 2, Config.Colors.Success)
                       else Toast.show("❌ " .. tostring(err):sub(1, 50), 3, Config.Colors.Error) end
                   end
               end, Parent = codeHeader })
    
    UI.button({ Text = "📋", Size = UDim2.new(0, 38, 0, 34), Position = UDim2.new(1, -88, 0, 5),
               OnClick = function() if State.source then Executor.clipboard(State.source) Toast.show("📋 Copied!", 2) end end, Parent = codeHeader })
    
    UI.button({ Text = "🔄", Size = UDim2.new(0, 38, 0, 34), Position = UDim2.new(1, -46, 0, 5),
               OnClick = function() if State.selected and State.selected:IsA("BaseScript") then Explorer.selectItem(State.selected) end end, Parent = codeHeader })
    
    State.metaLabel = UI.label({ Text = "", Size = UDim2.new(1, -12, 0, 20), Position = UDim2.new(0, 6, 0, 46), Color = Config.Colors.Muted, TextSize = 11, Parent = codePanel })
    
    local codeScroll = UI.scroll({ Size = UDim2.new(1, 0, 1, -70), Position = UDim2.new(0, 0, 0, 70), Parent = codePanel })
    State.codeLabel = Instance.new("TextBox")
    State.codeLabel.Size = UDim2.new(1, -16, 0, 0)
    State.codeLabel.Position = UDim2.new(0, 8, 0, 8)
    ...
    header.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging, dragStart, startPos = true, input.Position, State.mainFrame.Position
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            State.mainFrame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)
