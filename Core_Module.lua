...
-- --- State Variables ---
-- These are now global to this module's environment
...
activeXOffsetRaw = { mode = "px", value = 0 }
...
function computePixelXOffset(raw)
    if raw.mode == "px" then
        return raw.value
    else
...
function computePixelYOffset(raw)
    if raw.mode == "px" then
        return raw.value
    else
...
function safeSendMouseMove(x, y)
    if vmAvailable then
        pcall(function() VirtualInputManager:SendMouseMoveEvent(x, y, game, 0) end)
    end
end

function safeSendMouseButton(x, y, button, isDown)
    if vmAvailable then
...
function simulateClick(pixelPos)
...
function simulateSwipe(startPixel, endPixel, duration, curvatureFraction)
...
function doReplayActions(actionList)
...
-- --- Final Setup ---
-- Let Main.lua handle connections
